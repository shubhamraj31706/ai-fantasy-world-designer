...
"""
    ...
    def generate_story() -> Any:
        """
        Generate a full immersive short story (~700 words) from the current world
        in session. Uses all six world fields as source material so the story is
        consistent with the blueprint the user already generated.
        """
        world = _current_world()
        if not world:
            return jsonify({"error": "No world in session. Generate a world first."}), 400
        ...
        try:
            ...
            # Pull world data for the prompt
            name      = world.get("world_name", "an unnamed world")
            geo       = world.get("geography", [])
            history   = world.get("history", [])
            creatures = world.get("creatures", [])
            magic     = world.get("magic_system", [])
            hooks     = world.get("plot_hooks", [])
            tone      = world.get("tone_tags", [])

            geo_text  = "; ".join(str(g) for g in geo[:4])
            hist_text = "; ".join(str(h) for h in history[:3])
            magic_text = "; ".join(str(m) for m in magic[:3])
            hook_text  = str(hooks[0]) if hooks else ""
            tone_text  = ", ".join(str(t) for t in tone[:3]) if tone else "epic, mystical"
            creature_lines = "\n".join(
                f"- {c.get('name','')}: {c.get('description','')}"
                for c in creatures[:3] if isinstance(c, dict)
            )

            story_prompt = f"""You are a master fantasy storyteller. Write a complete, immersive short story (around 650-750 words) set in the world described below. 

WORLD DATA:
- World Name: {name}
- Geography: {geo_text}
- History: {hist_text}
- Magic System: {magic_text}
- Key Creatures: {creature_lines}
- Central Plot Hook: {hook_text}
- Tone: {tone_text}

STORY RULES:
1. Write a FULL story with a clear beginning, middle and end.
2. Feature at least one named protagonist who lives in this world.
3. Naturally weave in the geography, at least one creature, and the magic system.
4. The plot hook must drive the story's conflict or resolution.
5. Use simple, vivid language — no overly complex vocabulary.
6. VERY IMPORTANT: Write the entire story in the SAME LANGUAGE as the world name and lore above. If the world data is in Hindi, write in Hindi. If English, write in English.
7. No headers, no bullet points — just flowing narrative prose.
...
Write the story now:"""
            ...
            resp = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    temperature=0.85,   # slightly higher for creative storytelling
                    top_p=0.95,
                ),
            )
            story_text = (resp.text or "").strip()
            if not story_text:
                return jsonify({"error": "Gemini returned an empty story. Try again."}), 500

            return jsonify({"story": story_text, "world_name": name})

        except Exception as e:
            return jsonify({"error": "Story generation failed", "details": str(e)}), 500
